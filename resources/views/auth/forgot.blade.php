...
@section('content')
    <div class="col-12 d-flex justify-content-center">
        ...
        @if (session('status') && session('status')['status'] == 1)
            <div class="col-xs-12 col-md-8 col-xl-6 d-flex flex-wrap flex-column justify-content-center">
                <h1 class="mx-0 mt-4">Reset link</h1>
                Click on the link below to access a form to change your password.
                <div class="mt-3 alert alert-warning"><a href="{{ session('status')['message'] }}">Reset link 'sent to
                        email'</a></div>
                <div class="mt-3 p-0 lert alert-light" role="alert">
                    Note: this page is used as a replacement for an email. It is difficult to send emails in localhost, so I
        ...
        @else
            <form action="{{ route('forgot-pw') }}" method="POST"
                class="col-xs-12 col-md-8 col-xl-6 d-flex flex-wrap flex-column justify-content-center">
                @csrf
                ...
                <div class="d-flex flex-wrap flex-row justify-content-between">
                    ...
                    <a href="/login"> <button type="button" class="mt-3 px-4 btn btn-outline-secondary">Login</button></a>
                </div>
